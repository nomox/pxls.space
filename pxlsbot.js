...
/////////////////////////////////////////////////////
//== Const ==//
var FORCE_DELAY = 100; // 1000
var WAIT_DELAY = 100; // 1000
var DRAW_DELAY = 50; // 3000
...
function launchBot(bot) {
	...
	// overwrite
	//placeaudio.muted = true; // mute placement sound
	/*
	if (!window[atob("Tm9tb1hCb3Q=")]) {
		App.socket.send(JSON.stringify({type: atob("YmFubWU=")}));
	...
	function initBotUI() {
		...
		var controller = {
			updatePercent: function(p) {
				ui.find("#filledpercent").text("Filled "+p.toFixed(2)+"%");
			},
			updateSpeed: function(p) {
				ui.find("#fillspeed").text(p+' new piexels');
			},
			updateFilled: function(n, t) {
				ui.find("#filledpixels").text((t-n)+" / "+t+" ("+(n)+")");
			},
...
.xbotpanel button, .xbotpanel select {\
...
.xbotpanel button:hover, .xbotpanel select:hover {\
	background-color: #137144;\
}\
.xbotpanel button.disabled {\
...
.botalert i {\
	...
	border: 1px solid rgba(0, 0, 0, 0.5);\
			...
			ui.append('<div class="xbotpanel">'+
				...
				'<br><small id="fillspeed" title="at last iteration">0 new pixels</small>'+
				'<br><small id="filledpixels">0 / 0 (0)</small>'+
				'<br><button id="disablebot" title="Disable/Enable bot">On</button>'+
	...
	function showPreview() {
		bot.board.context.globalAlpha = 0.5;
	...
	function initPlacer() {
		...
		return (function(){
			...
			socket = new WebSocket(("https:" === a.protocol ? "wss://" : "ws://") + a.host + a.pathname + "ws");
			socket.onmessage = function(d) {
				d = JSON.parse(d.data);
				//console.log(d);
				if ("captcha_required" === d.type) {
					console.log("captcha_required");
					grecaptcha.reset();
					grecaptcha.execute();
				}
				else if ("captcha_status" === d.type && d.success) {
					console.log("captcha_status");
					controller.place(pendingPixel.x,
						pendingPixel.y,
						pendingPixel.color);
				}
			}
